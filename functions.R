# Code from Adam, modified by me at various points
...
filter_biomart_genes <- function(result, name){
  ...
  annots <-  getBM(attributes=c('ensembl_gene_id', 'external_gene_name'),filters = 'ensembl_gene_id', values = data, mart = ensembl)
  
  result <- merge(test, annots, by.x="row.names", by.y="ensembl_gene_id")
  ...
  sig_name = paste("results2/", name,"_sig.csv", sep="")
  sig_name_tsv = paste("results2/", name,"_sig.tsv", sep="")
  res_name = paste("results2/",name,"_res.csv", sep="")
  res_name_tsv = paste("results2/", name,"_res.tsv", sep="")
  write_csv(sig, sig_name)
  write_csv(res, res_name)
  write_tsv(sig, sig_name_tsv)
  write_tsv(res, res_name_tsv)
...
ensembl_to_symbol_biomart <- function(dataframe, ensembl_column){
  ...
  result <- merge(dataframe, annots, by.x=ensembl_column, by.y="ensembl_gene_id")
  return(result)
}
